...
class create_trainer(object):
    ...
    def do_train(self, epoch, market_loader, loss_mode):
        ...
        for batch_idx, data in enumerate(market_loader):
            
            self.reset_grad()
            
            img_gray, l_market, _, _, att = data
            img_gray = img_gray.cuda()
            l_market = l_market.cuda()
            ...
            if loss_mode == 'softmax':
                loss = self.xent(clcs,l_market) + loss_att
                           
            elif loss_mode == 'triplet_softmax':   
                loss = self.xent(clcs,l_market) + self.triplet(feat_1, l_market) + self.triplet(feat_2, l_market) + loss_att 
            ...
            if batch_idx%10 ==0:
                print('Epoch: [{}][{}/{}], Loss: {:.2f}'.format(epoch, batch_idx, len(market_loader), loss.cpu().detach().numpy()))
    ...
    def save_model(self,epoch):
        ...
        state = {
            'Embed_net_1': self.Embed_net.state_dict(),
            'Embed_net_2': self.Embed_net.state_dict(),
            'Classify_net': self.Classify_net.state_dict(),
            'A_net': self.A_net.state_dict(),
            'epoch': epoch
        }
        torch.save(state, self.args.model_path + 'market' + '_epoch_{}.t'.format(epoch))
